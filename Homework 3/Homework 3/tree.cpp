...
{
    if (n2 <= 0)    //if n2 is empty, all elements have been found, so return 1 possible outcome.
        return 1;
    
    if (n1 <= 1)
    {
        if (n2 == 1 && a1[0] == a2[0])      //last elements were equal, so return 1.
            return 1;
        else
            return 0;                       //nothing found, return 0.
    }
    
    if (a1[0] == a2[0])                     //possibility found, branch and continue
        return countContains(a1+1, n1-1, a2+1, n2-1) + countContains(a1+1, n1-1, a2, n2);
    else                                    //nothing found, continue
...
void order(string a[], int n)
{
    ...
    int divider = n/2;  //find the middle
    int firstNotLess;
    int firstGreater;
    
    divide(a, n, a[divider], firstNotLess, firstGreater); //use middle as divider for divide fxn.
    
    order(a, firstNotLess);                               //repeat with left side.
    order(a + firstGreater, n - firstGreater);            //repeat with right side.
}
