...
bool pathExists(char maze[][10], int sr, int sc, int er, int ec)
{
    ...
    while (!coordStack.empty())         //creating a loop that runs as long as stack isn't empty
    {
        ...
        else
        {
            ...
            //Check SOUTH.
            if (cur.r()+1 <= 10 && maze[cur.r()+1][cur.c()] != 'X' && maze[cur.r()+1][cur.c()] != '#')  //If coordinate is in range, is not a wall, and hasn't been discovered.
            {
                coordStack.push(Coord(cur.r()+1, cur.c()));         //Push discovered coord to stack
                maze[cur.r()+1][cur.c()] = '#';                     //Update Maze
            }
            //Check EAST.
            if (cur.c()+1 <= 10 && maze[cur.r()][cur.c()+1] != 'X' && maze[cur.r()][cur.c()+1] != '#')   //If coordinate is in range, is not a wall, and hasn't been discovered.
            {
...
int main()  // Test case
{
    ...
    if (pathExists(maze2, 2,2, 7,7)) // why does this work? 
        cout << "Solvable!" << endl;
    else
        cout << "Out of luck!" << endl;
